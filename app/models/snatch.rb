...
class Snatch < ActiveRecord::Base
  def self.snatch_qq_vip_video(name)
    ...
    page.css("div[class='_playlist']").first.css("div[class='item']").each do |item|
      ...
      a << {name: item.css('a')[0].text.to_i == 0 ? item.css('a')[0].text : "第#{item.css('a')[0].text}集",href: "http://120.25.245.217:8080/homes?url=#{item.css('a')[0]['href'].gsub('v.qq.com','film.qq.com')}"}
    end
