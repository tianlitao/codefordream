...
class Snatch < ActiveRecord::Base
  def self.snatch_qq_vip_video(name)
    ...
    page.css("div[class='_playlist']").first.css("div[class='item']").each do |item|
      ...
      a << {name: item.css('a')[0].text.to_i == 0 ? item.css('a')[0].text : "第#{item.css('a')[0].text}集",href: "http://tianlitao.ittun.com/homes?url=#{item.css('a')[0]['href'].gsub(/html?.*/,'html')}"}
    end
    page.css("div[class='_playlist']").first.css("div[class='result_btn_line']").each do |item|
      a << {name:  item.css('a')[0].text == '免费试看' ? '免费观看' : item.css('a')[0].text ,href: "http://tianlitao.ittun.com/homes?url=#{item.css('a')[0]['href'].gsub('v.qq.com','film.qq.com').gsub(/html?.*/,'html')}"}
    end
    a
  end
end
